...
Tuy nhiên, các mô hình Machine Learning thực tế thường gặp phải vấn đề **Data Drift / Model Drift** — chất lượng dự đoán suy giảm theo thời gian do thói quen người dùng thay đổi. Hơn nữa, việc huấn luyện và triển khai mô hình thủ công từ Jupyter Notebook lên môi trường Production tiêu tốn nhiều thời gian, dễ phát sinh lỗi vận hành và thiếu các lớp bảo vệ tường lửa cho các API công khai.

Workshop này sẽ xây dựng một **Hệ thống MLOps Tự động hóa Khép kín (End-to-End Automated MLOps Platform)** trên nền tảng **AWS Cloud**, được bảo mật và tối ưu hóa bằng **Amazon CloudFront** và **AWS WAF**, giúp giải quyết triệt để các thách thức trên.

---

## Mục tiêu Workshop
Sau khi hoàn thành bài lab này, bạn sẽ nắm vững và triển khai được:
1. **Bảo mật & Tối ưu API (CloudFront & AWS WAF):** Cấu hình Amazon CloudFront làm Edge Location đại diện phía trước API Gateway để tăng tốc độ phản hồi API, kết hợp kích hoạt tường lửa AWS WAF với quy tắc Rate Limiting để ngăn chặn các truy cập bất thường và bảo vệ hạ tầng backend.
2. **Dự đoán thời gian thực (Real-time Inference):** Tích hợp Amazon API Gateway, AWS Lambda và AWS SageMaker Serverless Endpoint để xử lý request và trả về xác suất rời bỏ dịch vụ tức thì với chi phí tối ưu (0đ khi không có lưu lượng).
3. **Kích hoạt tự động (Event-Driven Trigger):** Tự động phát hiện khi Admin tải dữ liệu mới lên Amazon S3, kiểm tra Data Drift và khởi chạy luồng Retrain.
4. **Luồng làm việc MLOps (SageMaker Pipeline - 4 bước):**
...
5. **Triển khai tự động (Continuous Deployment - CD):** Sử dụng Amazon EventBridge lắng nghe trạng thái Approved từ Model Registry để kích hoạt AWS Lambda Deployer tự động cập nhật Serverless Endpoint mà không gây gián đoạn dịch vụ (Zero-Downtime Deployment).
6. **Giám sát & Báo động (Monitoring & Alerting):** Lưu trữ log tập trung qua CloudWatch Logs, thiết lập CloudWatch Alarm và gửi email cảnh báo tự động về hòm thư qua Amazon SNS.

---

## Sơ đồ Kiến trúc Hệ thống (Architecture Diagram)

![Sơ đồ Kiến trúc MLOps AWS](/images/5-Workshop/5.1-Workshop-overview/image.png)

### Các Dịch vụ AWS Sử Dụng:
- **Amazon S3:** Lưu trữ Dữ liệu thô, Dữ liệu đã xử lý và Model Artifacts.
- **Amazon CloudFront & AWS WAF:** Cung cấp CDN làm cổng đại diện công khai (Edge Location), tăng tốc độ truyền tải API và áp dụng quy tắc tường lửa (Rate Limiting) chống tấn công DoS/DDoS.
